    ...
    while r.gintsts().read().rxflvl() {
        ...
        match status.pktstsd() {
            ...
            vals::Pktstsd::OUT_DATA_RX => {
                ...
                // PATCH: endpoints other than 0 accumulate packets until the
                // transfer completes (OUT_DATA_DONE); endpoint 0 keeps the
                // original one-packet-then-wake behaviour.
                let current = ep.out_size.load(Ordering::Acquire);
                let offset = if ep_num == 0 || current == EP_OUT_BUFFER_EMPTY {
                    0
                } else {
                    current as usize
                };
                let capacity = state
                    ...
                    .map(|e| e.out_buffer_size as usize)
                    .unwrap_or(0);
                let accepting = if ep_num == 0 {
                    current == EP_OUT_BUFFER_EMPTY
                } else {
                    !ep.out_done.load(Ordering::Acquire) && offset + len <= capacity
                };

                if accepting {
                    // SAFETY: the buffer was allocated with `out_buffer_size` bytes and the
                    // bounds were checked above.
                    let buf = unsafe { core::slice::from_raw_parts_mut((*ep.out_buffer.get()).add(offset), len) };
                    ...
                    ep.out_size.store((offset + len) as u16, Ordering::Release);
                    if ep_num == 0 {
                        ep.out_waker.wake();
                    }
                } else {
            ...
            vals::Pktstsd::OUT_DATA_DONE => {
                ...
                // PATCH: the whole (possibly multi-packet) transfer is in the buffer.
                if ep_num != 0 {
                    let ep = &state.ep_states[ep_num];
                    if ep.out_size.load(Ordering::Acquire) == EP_OUT_BUFFER_EMPTY {
                        ep.out_size.store(0, Ordering::Release);
                    }
...
/// PATCH (gud-esp32s3): bulk OUT endpoints receive up to this many packets
/// per transfer instead of one, so the task is woken once per transfer
/// rather than once per 64-byte packet. Endpoint 0 keeps the original
/// single-packet behaviour because the control pipe depends on it.
const OUT_PACKETS_PER_XFER: u16 = 128;
/// RX FIFO room reserved per bulk OUT endpoint, in packets, so the interrupt
/// handler can lag a little without the core NAKing.
const OUT_RX_FIFO_PACKETS: u16 = 4;
...
struct EpState {
    ...
    out_size: AtomicU16,
    /// PATCH: set by the interrupt handler when a multi-packet OUT transfer completes.
    out_done: AtomicBool,
...
struct EndpointData {
    ...
    max_packet_size: u16,
    fifo_size_words: u16,
    /// PATCH: bytes reserved in the OUT buffer (mps, or mps * packets per transfer).
    out_buffer_size: u16,
}
...
impl<'d> Driver<'d> {
    ...
    fn alloc_endpoint<D: Dir>(
        ...
        // PATCH: bulk OUT endpoints get room for several packets per transfer.
        let multi_packet = D::dir() == Direction::Out && ep_type == EndpointType::Bulk;
        let out_buffer_size = if multi_packet {
            max_packet_size * OUT_PACKETS_PER_XFER
        } else {
            max_packet_size
        };

        if D::dir() == Direction::Out {
            if self.ep_out_buffer_offset + out_buffer_size as usize > self.ep_out_buffer.len() {
        ...
        let fifo_size_words = match D::dir() {
            Direction::Out if multi_packet => (max_packet_size * OUT_RX_FIFO_PACKETS + 3) / 4,
        ...
        if D::dir() == Direction::Out {
            ...
            self.ep_out_buffer_offset += out_buffer_size as usize;
        }
...
impl<'d> embassy_usb_driver::EndpointOut for Endpoint<'d, Out> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, EndpointError> {
        ...
        poll_fn(|cx| {
            ...
            // PATCH: for endpoints other than 0 the transfer is armed here, when
            // the buffer size is known, instead of after the previous read. Bulk
            // endpoints take up to OUT_PACKETS_PER_XFER packets in one go.
            if index != 0 {
                let armed = self.regs.doepctl(index).read().epena();
                let done = self.state.out_done.load(Ordering::Acquire);
                let size = self.state.out_size.load(Ordering::Acquire);
                if !armed && !done && size == EP_OUT_BUFFER_EMPTY {
                    let mps = self.info.max_packet_size;
                    let max_packets = if self.info.ep_type == EndpointType::Bulk {
                        OUT_PACKETS_PER_XFER
                    } else {
                        1
                    };
                    let packets = ((buf.len() / mps.max(1) as usize) as u16).clamp(1, max_packets);
                    trace!("read ep={:?} arming {} packets", self.info.addr, packets);
                    critical_section::with(|_| {
                        self.regs.doeptsiz(index).modify(|w| {
                            w.set_xfrsiz((packets * mps) as _);
                            w.set_pktcnt(packets);
                        });
                ...
                // Release buffer before returning so the next read can arm.
                let result = if len as usize > buf.len() {
                    Err(EndpointError::BufferOverflow)
                } else {
                    // SAFETY: exclusive access ensured by `out_done`
